...
import com.wanqiao.mogao.project.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    public TableDataInfo listAllUser(SysUser user)
    {
        List<SysUser> list = userService.selectUserList(user);
        return getDataTable(list);
    }
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user)
    {
        try
        {
            user.setPassword(MoGaoAES.getInstance().Decrypt(user.getPassword()));
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public AjaxResult resetPwd(@RequestBody SysUser user)
    {
        ...
        try
        {
            user.setPassword(SecurityUtils.encryptPassword(MoGaoAES.getInstance().Decrypt(user.getPassword())));
        }
        catch (Exception e)
        {
            e.printStackTrace();
            return AjaxResult.error("AES解密失败");
        }
        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
        user.setUpdateBy(SecurityUtils.getUsername());
