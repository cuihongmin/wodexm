 ...
 * @description 农历、阳历互转(来源于网络)
 * @author yekairush $<a href="mailto:yekairush@163.com.com">联系作者</a>$
 * @version $id:ChinaCalendar.java,Revision:v1.0,Date:2011-6-1 下午02:22:32 $
 */
...
class Week {
	...
	private static final Logger LOGGER = Logger.getLogger(com.cui.common.utils.ChinaCalendar.class);
	...
	public Week(int w) {
		if ((w > 6) || (w < 0)) {
			LOGGER.info("Week out of range, I think you want Sunday");
			this.iWeek = 0;
		} else
			this.iWeek = w;
	}
...
class MyDate {
	...
	private static int checkYear(int iYear) {
		if ((iYear > 1901) && (iYear < 2050))
			...
			LOGGER.info("The Year out of range, I think you want 1981");
			return 1981;
		}
	}
	...
	public boolean equals(com.cui.common.utils.MyDate md) {
		return ((md.iDay == this.iDay) && (md.iMonth == this.iMonth) && (md.iYear == this.iYear));
	}
}

// 阳历日期类,继承自定义日期
class SolarDate extends com.cui.common.utils.MyDate {

	private static final Logger LOGGER = Logger.getLogger(com.cui.common.utils.ChinaCalendar.class);

	private static int checkMonth(int iMonth) {
		if (iMonth > 12) {
			LOGGER.info("Month out of range, I think you want 12 ");
			return 12;
		} else if (iMonth < 1) {
			LOGGER.info("Month out of range, I think you want 1 ");
			return 1;
		} else
			return iMonth;
	}
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = com.cui.common.utils.ChinaCalendar.iGetSYearMonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
			LOGGER.info("Day out of range, I think you want " + iMonthDays+ " ");
			return iMonthDays;
		} else if (iDay < 1) {
			LOGGER.info("Day out of range, I think you want 1 ");
			return 1;
		} else
			return iDay;
	}
	...
	public com.cui.common.utils.Week toWeek() {
		int iOffsetDays = 0;
		for (int i = 1901; i < iYear; i++) {
			if (com.cui.common.utils.ChinaCalendar.bIsSolarLeapYear(i))
				iOffsetDays += 366;
			else
				iOffsetDays += 365;
		}
		iOffsetDays += com.cui.common.utils.ChinaCalendar.iGetSNewYearOffsetDays(iYear, iMonth, iDay);
		return new com.cui.common.utils.Week((iOffsetDays + 2) % 7);
	}

	public com.cui.common.utils.LunarDate toLunarDate() {
		
		int iYear, iMonth, iDay, iDate;
		com.cui.common.utils.LunarDate ld;
		iDate = Integer.parseInt(com.cui.common.utils.ChinaCalendar.sCalendarSolarToLundar(this.iYear, this.iMonth, this.iDay));
		iYear = iDate / 10000;
		iMonth = iDate % 10000 / 100;
		iDay = iDate % 100;
		ld = new com.cui.common.utils.LunarDate(iYear, iMonth, iDay);
		return ld;
	}
}

// 阴历日期类,继承自定义日期类
class LunarDate extends com.cui.common.utils.MyDate {
	...
	private static int checkMonth(int iYear, int iMonth) {
		if ((iMonth > 12) && (iMonth == com.cui.common.utils.ChinaCalendar.iGetLLeapMonth(iYear) + 12)) {
			return iMonth;
		} else if (iMonth > 12) {
			LOGGER.info("Month out of range, I think you want 12 ");
			return 12;
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = com.cui.common.utils.ChinaCalendar.iGetLMonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
			LOGGER.info("Day out of range, I think you want " + iMonthDays+ " ");
			return iMonthDays;
		} else if (iDay < 1) {
	...
	public com.cui.common.utils.CnWeek toWeek() {
		int iOffsetDays = 0;
		for (int i = 1901; i < iYear; i++)
			iOffsetDays += com.cui.common.utils.ChinaCalendar.iGetLYearDays(i);
		iOffsetDays += com.cui.common.utils.ChinaCalendar.iGetLNewYearOffsetDays(iYear, iMonth, iDay);
		return new com.cui.common.utils.CnWeek((iOffsetDays + 2) % 7);
	}

	public com.cui.common.utils.ChineseEra toChineseEra() {
		return new com.cui.common.utils.ChineseEra(iYear);
	}

	public com.cui.common.utils.SolarDate toSolarDate() {
		int iYear, iMonth, iDay, iDate;
		com.cui.common.utils.SolarDate sd;
		iDate = Integer.parseInt(com.cui.common.utils.ChinaCalendar.sCalendarLundarToSolar(this.iYear, this.iMonth, this.iDay));
		iYear = iDate / 10000;
		iMonth = iDate % 10000 / 100;
		iDay = iDate % 100;
		sd = new com.cui.common.utils.SolarDate(iYear, iMonth, iDay);
		return sd;
	}
}

class CnWeek extends com.cui.common.utils.Week {
